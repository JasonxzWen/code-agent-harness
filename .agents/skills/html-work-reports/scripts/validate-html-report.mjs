...
function validateStatic(html) {
  ...
  add(
    checks,
    "report-root",
    html.includes("data-html-work-report"),
    "missing data-html-work-report root",
    issues
  );
  add(
    checks,
    ...
    "report appears empty or too small",
    issues
  );
  add(
    checks,
    ...
    "unsafe script, handler, or protocol found",
    issues
  );
  ...
  if (html.includes('data-section-type="markdown"')) {
    add(
      checks,
      "markdown-rendered",
      html.includes("rendered-markdown") ||
        (runtime &&
          html.includes("data-rich-markdown") &&
          html.includes("data-source-fallback") &&
          runtimePins),
    ...
    add(
      ...
      html.includes("<table>") ||
        html.includes("<ul>") ||
        (runtime && html.includes("data-rich-markdown")),
  ...
  if (html.includes('data-section-type="mermaid"')) {
    add(
      ...
      html.includes("<svg") ||
        (runtime &&
          html.includes("data-rich-mermaid") &&
          html.includes("data-mermaid-source") &&
          runtimePins),
  ...
  if (html.includes('data-section-type="code"')) {
    add(
      checks,
      "code-highlighted",
      html.includes('class="hljs') ||
        (runtime && html.includes("language-") && runtimePins),
  ...
  if (html.includes('data-section-type="diff"')) {
    add(
      checks,
      "diff-rendered",
      html.includes("diff-panel") &&
        (html.includes("diff-added") || html.includes("diff-removed")),
  ...
  add(
    checks,
    "evidence-present",
    html.includes("data-evidence") && html.includes("data-evidence-kind="),
  ...
  add(
    ...
    html.includes("data-verification-status="),
    "missing verification status blocks",
    issues
  );
  add(
    checks,
    "interactive-controls",
    html.includes("data-filter-target") &&
      html.includes("data-tab-group") &&
      html.includes("data-copy-from"),
  ...
  if (runtime) {
    add(
      ...
      "runtime-pins",
      runtimePins,
      "runtime mode does not declare pinned dependencies",
      issues
    );
    add(
      checks,
      "runtime-fallbacks",
      html.includes("data-source-fallback"),
      "runtime sections lack source fallback",
      issues
    );
  } else {
    add(
      ...
      !html.includes("https://cdn.jsdelivr.net"),
      "default report includes CDN dependency",
      issues
    );
  }
...
async function validateBrowser(file, options) {
  ...
  try {
    ...
    const hiddenPanels = await page
      .locator("[data-tab-panel-group][hidden], [data-filter-target][hidden]")
      .count();
...
if (
  ...
  process.argv[1]?.endsWith("validate-html-report.mjs")
) {
  await main();
}
