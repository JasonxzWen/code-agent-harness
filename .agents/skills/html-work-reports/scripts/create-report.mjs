...
const richRuntimeCssPath = path.join(
  skillDir,
  "assets",
  "components",
  "rich-render-runtime.css"
);
const richRuntimeJsPath = path.join(
  skillDir,
  "assets",
  "components",
  "rich-render-runtime.js"
);
...
const templateMeta = {
  "implementation-handoff": {
    label: "实现交接",
    useCase: "完成实现、验证门禁、文件证据、风险和下一步",
    accent: "#2563eb"
  },
  "review-findings": {
    label: "审查发现",
    useCase: "代码或文档审查、severity 过滤、片段、owner 和行动项导出",
    accent: "#c2414b"
  },
  "research-explainer": {
    label: "调研解释",
    useCase: "调研综合、架构 walkthrough、来源证据和图示",
    accent: "#0f766e"
  },
  "decision-matrix": {
    label: "决策矩阵",
    useCase: "方案比较、推荐结论、取舍、风险和待确认问题",
    accent: "#b7791f"
  }
};
...
function slugify(value) {
  ...
  return `html-work-report-${hash.toString(36)}`;
}
...
function highlightLine(line, language) {
  ...
  if (["javascript", "js", "typescript", "ts"].includes(language)) {
    html = html.replace(
      /\b(async|await|const|let|var|return|function|export|import|from|if|else|try|catch|new)\b/g,
    ...
    html = html.replace(
      /(&quot;[^&]*?&quot;)(\s*:)?/g,
      '<span class="hljs-string">$1</span>$2'
    );
    html = html.replace(
    ...
    html = html.replace(
      /\b(bun|node|npm|git|openspec|powershell)\b/g,
      '<span class="hljs-keyword">$1</span>'
    );
  }
  return html;
...
async function renderMermaidSvg(source, title, options) {
  ...
  return fallbackMermaidSvg(
    source,
    title,
    "未请求浏览器 Mermaid 渲染；已生成 deterministic inline SVG fallback。"
  );
}
...
function fallbackMermaidSvg(source, title, message) {
  ...
  const renderedLines = lines
    .map(
      (line, index) =>
        `<text x="34" y="${88 + index * 24}" font-size="14" fill="#172033">${escapeHtml(line)}</text>`
    )
    .join("");
...
async function renderMermaidSection(section, mode, index, options) {
  ...
  return `<section class="panel diagram-panel mermaid-evidence" id="${sectionId(section.title)}" data-section-type="mermaid" data-source-fallback>
    <div class="split-row"><h2>${escapeHtml(section.title)}</h2><span class="rich-status" data-state="ready">Mermaid inline SVG 已生成</span></div>
    ${svg}
    <details><summary>Mermaid source</summary><pre data-mermaid-source>${escapeHtml(section.content)}</pre></details>
  </section>`;
}
    ...
    <div class="evidence-grid focus-field" data-focus-field="${target}">
      ${items
        .map(
          (
            item
          ) => `<article class="interactive-card evidence-card evidence-spotlight" data-evidence-spotlight data-filter-target="${target}" data-filter-value="${escapeAttr(item.group || "item")}" data-search-target="${target}">
    ...
    <div class="metric-grid focus-field">
      ${options
        .map(
          (
            option
          ) => `<article class="interactive-card evidence-card evidence-spotlight" data-evidence-spotlight>
...
async function renderSection(section, mode, index, input, options) {
  ...
  if (section.type === "mermaid")
    return renderMermaidSection(section, mode, index, options);
  ...
  return `<div class="evidence-grid" data-evidence>
    ${items
      .map(
        (
          item
        ) => `<article class="interactive-card evidence-card evidence-spotlight" data-evidence-spotlight data-evidence-kind="${escapeAttr(item.kind)}">
  ...
  return `<div class="evidence-grid" data-verification>
    ${(items || [])
      .map(
        (
          item
        ) => `<article class="interactive-card evidence-card evidence-spotlight" data-evidence-spotlight data-verification-status="${escapeAttr(item.status)}">
      <div class="split-row"><span class="meta">验证</span><span class="status-pill ${statusClass(item.status)}">${escapeHtml(item.status)}</span></div>
      <h3>${escapeHtml(item.label)}</h3>
...
function renderRuntimeDependencies(mode) {
  ...
  const pins = Object.entries(runtimePins).map(
    ([name, version]) => `${name}@${version}`
  );
  return `<section class="panel" data-runtime-dependencies>
    <h2>Runtime 依赖</h2>
    <div class="evidence-grid">
      ${pins.map((pin) => `<article class="evidence-card" data-runtime-dependency="${escapeAttr(pin)}"><strong>${escapeHtml(pin)}</strong><p>可选的 post-load 增强；source fallback 仍保持可见。</p></article>`).join("\n")}
    </div>
  </section>`;
}
...
function validateInput(input) {
  ...
  if (input.renderMode && !["pre-rendered", "runtime"].includes(input.renderMode))
    errors.push("renderMode must be pre-rendered or runtime.");
  if (input.template && !templateMeta[input.template])
    errors.push(`Unknown template: ${input.template}`);
...
async function createReport(input, options = {}) {
  ...
  for (let index = 0; index < input.sections.length; index += 1) {
    sections.push(
      await renderSection(input.sections[index], mode, index, input, options)
    );
  }
  ...
  const nav = input.sections
    .map(
      (section) =>
        `<a href="#${sectionId(section.title)}">${escapeHtml(section.title)}</a>`
    )
    .join("");
  ...
  <main class="report-shell">
    <header class="report-hero">
      <div class="title-row">
        ...
        <span class="status-pill ${statusClass(input.status)}">状态：${escapeHtml(input.status)}</span>
      </div>
      <div class="lede-grid">
    ...
    <div class="report-section-stack">
      ...
      <section class="panel" id="next-actions" data-section-type="actions"><div class="split-row"><h2>下一步</h2><button data-copy-from="#next-action-list">复制行动项</button></div><ul id="next-action-list">${(input.nextActions || []).map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul></section>
    </div>
  </main>
        ...
        JSON.stringify(
          {
            ok: true,
            outputPath,
            renderMode: input.renderMode || "pre-rendered",
            template: input.template || "implementation-handoff"
          },
          null,
          2
        )
...
if (
  ...
  process.argv[1]?.endsWith("create-report.mjs")
) {
  await main();
}
