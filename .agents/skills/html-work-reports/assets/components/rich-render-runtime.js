...
(function () {
  ...
  function renderMarkdown() {
    ...
    document.querySelectorAll("[data-rich-markdown]").forEach(function (node) {
      ...
      if (!parser || typeof parser.parse !== "function") {
        setStatus(statusId, "fallback", "Markdown runtime 缺失，显示 source");
        return;
      }

      if (!purifier && !trusted) {
        setStatus(statusId, "error", "Markdown 未渲染：sanitizer 缺失");
        return;
      }
  ...
  async function renderMermaid() {
    if (!window.mermaid) {
      setStatus("mermaid", "fallback", "Mermaid runtime 缺失，显示 source");
      return 0;
    }
  ...
  function highlightCode() {
    if (!window.hljs || typeof window.hljs.highlightElement !== "function") {
      setStatus("code", "fallback", "Highlighter 缺失，显示 plain code");
      return 0;
    }
