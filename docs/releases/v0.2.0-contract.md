...
v0.2 必须有用户级完整 workflow 验收，不能只依赖 unit tests。

| 场景               | 通过条件                                                                     |
| ------------------ | ---------------------------------------------------------------------------- |
| Patch approval E2E | 从 provider 提出 `apply_patch` 到 permission preview、批准、写入、最终回答。 |
| Denial / abort E2E | 用户拒绝或 abort 时，工作区不被写入，并产生结构化结果或 aborted state。      |
| Trace evidence     | trace/事件包含 bounded preview metadata 和 tool result metadata。            |
| Worktree evidence  | 批准后目标文件被修改，但没有 staged diff、新 commit 或 push。                |
...
```txt
Can the harness apply approved text patches while rejecting unsafe or stale patch scenarios?
```
...
| 指标                   | 最低接受线                                                                             |
| ---------------------- | -------------------------------------------------------------------------------------- |
| Required safety matrix | contract 中列出的 path、secret、binary/mode/symlink、dirty、stale、size 场景全部拒绝。 |
| Approval boundary      | 未批准、拒绝、abort 场景不得写入。                                                     |
| Apply success          | 至少一个 fixture repo 中的 approved patch 能完成写入并进入最终回答。                   |
| Git boundary           | approved apply 不得 stage、commit、push。                                              |
| Peer baseline          | Codex、Claude Code、opencode 等竞品的公开权限/写入边界已对照，并说明本项目取舍。       |
