# v0.1.0 Minimal Coding Agent 发布说明
...
- v0.1 是 read-only，不能 edit 或 patch files。
- `run_command` 有意限制在很小的 read-only allowlist。
- build 会产出本地 Bun CLI bundle，并 externalize runtime dependencies；在 `bun install` 后使用。
- OpenAI 是第一个真实 provider。当前 boundary 之外的 multi-provider behavior 仍是未来工作。
- OpenAI provider tool results 尚未通过 provider-native `tool_call_id` messages 映射。见 `docs/agent/openai-provider.md`。
- Live provider smoke 需要 `OPENAI_API_KEY`。
...
- OpenAI provider 的 tool result 当前还不是 provider-native
  `tool_call_id` 映射；限制记录见 `docs/agent/openai-provider.md`。
...
`v0.1.0` tag 指向 commit `1e794c2`。

- Release verification 已完成本地候选版本。
- `v0.1.0` tag 已创建；对应 commit `1e794c2`。
