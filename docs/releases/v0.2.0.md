...
| Feature               | User need / primary scenario                                                                         | Industry practice                                                                                      | Our approach                                                                         | Why                                                                          | How                                                                                                                                 | E2E acceptance                                                                        | Benchmark                                                                                                                                   |
| --------------------- | ---------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
...
flowchart TD
  ...
  E --> F["Preview in permission prompt"]
...
| `bun run smoke:live`                                                               | not run; optional live provider smoke requires `OPENAI_API_KEY` |

## E2E 验收

| Scenario           | Command / steps                                  | Fixture / input                                  | Expected evidence                                                                                                                  | Result |
| ------------------ | ------------------------------------------------ | ------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------- | ------ |
| Patch approval E2E | `bun run smoke`                                  | temporary git repo created by `scripts/smoke.ts` | provider requests `apply_patch`, permission gate sees preview, approval writes `src/index.ts`, final answer mentions modified file | pass   |
| Denial path        | `bun test packages/tools/test/tools.test.ts`     | temp git repo with `src/index.ts`                | `apply_patch` without approval returns `permission_denied` and file remains unchanged                                              | pass   |
| Abort path         | `bun test packages/core/test/agent-loop.test.ts` | permission gate that never resolves              | abort stops before tool execution and emits aborted state                                                                          | pass   |
| Worktree boundary  | `bun test packages/tools/test/tools.test.ts`     | approved patch in temp git repo                  | file changes, staged diff is empty, commit count stays unchanged                                                                   | pass   |

## Benchmark

| Question                                                                                     | Metric                           | Dataset / fixture                                 | Command / artifact                                   | Threshold                                                                              | Result | Caveat                                                                       |
| -------------------------------------------------------------------------------------------- | -------------------------------- | ------------------------------------------------- | ---------------------------------------------------- | -------------------------------------------------------------------------------------- | ------ | ---------------------------------------------------------------------------- |
| Can the harness apply approved text patches while rejecting unsafe or stale patch scenarios? | Required safety matrix pass rate | tool tests with temp git repos and patch fixtures | `bun test packages/tools/test/tools.test.ts`         | all required policy scenarios pass                                                     | pass   | This is a local harness benchmark, not a public ranking.                     |
| Same                                                                                         | End-to-end patch success         | deterministic smoke temporary repo                | `bun run smoke`                                      | provider-to-final-answer patch workflow succeeds                                       | pass   | Live OpenAI path was not benchmarked.                                        |
